...
export class AppComponent implements OnInit {
  ...
  setLayers() {
    // Adds a layer
    //var ciLayer = (<any>L).canvasMarker({}).addTo(this.map);


    let points = 1500;
    let radius = 10;

    let step = ((3.14159265 * 2) / points);
    let x, y, current = 0;
    for (let i = 0; i < points; i++)
    {
        x = Math.sin(current) * radius;
        y = Math.cos(current) * radius;
        (<any>L).canvasMarker({lat: x, lng: y}, {icon: this.myIcon, pane: 'foo'}).addTo(this.map);
        //ciLayer.addMarker(new L.Marker({lat: x, lng: y}, {icon: this.myIcon, pane: 'foo'}));
